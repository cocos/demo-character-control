import { _decorator, Component, MeshRenderer, primitives, utils, Material, Vec4, gfx } from 'cc';
...
export class wireFrame extends Component {
    ...
    onEnable() {
        ...
        if (model && model.mesh && model.mesh.subMeshCount > 0) {
            for (let i = 0; i < model.mesh.subMeshCount; i++) {
                const newModel = this.addComponent(MeshRenderer)!;
                const geo = {
                    positions: model.mesh.renderingSubMeshes[i].geometricInfo.positions.slice(),
                    indices: model.mesh.renderingSubMeshes[i].geometricInfo.indices!.slice(),
                }
